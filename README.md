# week3-hotel-simulator
Web application to handle guest check-ins, manage room assignments, and display an organized list of clients staying at a virtual hotel.

## 1: Create a Hotel Simulator Web Application
- [] Design HTML layout styling with:
  - [] Check-In Form
  - [] Guest List 
- [] Make it pretty!

## 2: Implement Check-In Form
- [] Include fields for:
  - [] Guest Name (**text input**)
  - [] Room Number (**number input**)
  - [] Stay Duration (**number input**)
  - [] *Optional* Whatever else I'd like to add
- [] Add **validation** to ensure all fields are filled before submission

## 3: Add Room Availability Logic
- [] Use JS to ensure no two guests can be checked into the same room
- [] If a room is occupied - display an error message prompting the user to select a different room (can be *alert*, *badge*, *simple message*)

## 4: Display Guest List
- [] Dynamically render a list of guests using JS. Each guest should be displayed in a card format showing:
  - [] Guest Name
  - [] Room Number
  - [] Stay Duration

## 5: Prepopulate the Guest List
- [] Use JS array to store initial guest data and render it upon page load

## 6: Handle Form Submission
- [] Assign form submission behaviors
  - [] Validate Inputs
  - [] Check for room availability
  - [] Add new guest to guest list and dynamically update their display
  - [] Clear form inputs

## 7: Code Requirements
- [] Use a constructor function to create Guest Objects
- [] Write clean, readable, and **commented** code
- [] Avoid using external JKS frameworks or libraries for functionality - only things we've learned in class.


- [] Upload to GitHub and deploy w/ GitHub Pages
  